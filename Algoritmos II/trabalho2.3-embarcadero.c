...
//	G u l l e n h e i m U l t i m a t e T e a m	
 char vet[][max] = {"<GUT23>Cadastra os Jogadores", "<GUT23>Mostra os Jogadores", 
 					"<GUT23>Consulta Overall","<GUT23>Consulta Pais",  "<GUT23>Altera Atributos do Jogador", 
 					"<GUT23>Coloca jogador no Banco", "<GUT23>Mostra jogadores no banco", "<GUT23>Tira os jogadores do banco" ,
 					"<GUT23>Demite Jogador","<GUT23>Ajuda - F1","<GUT23>Sair - esc"};
...
void card_mostra(char arquivo_nome[]){
	...
	if(jogador.existe==1){
       ...
       if(cont<=0)cont=1;
		...
		escreveTextoss(62, 22,"   ", 7);
		escreveTextoss(35, 23, "Pressione <- ou -> para rever, CIMA ou BAIXO para continuar", 8);
}
		  y=getch();	
	   //"--G--U--T--2--0--2--3--\n"
}
...
funcao card_mostrabanco*/
    ...
    escreveTextoss(35, 24,"Digite o numero da camisa do jogador que ira por no banco: ", 3 );
    scanf("%d", &busca_camisa);
    ...
    if(!achou) escreveTextoss(43, 26,"Nenhum jogador tem o numero digitado", 3 );
    
    escreveTextoss(20, 27,"--G--U--T--2--0--2--3--> Pressione as setas para continuar <--G--U--T--2--0--2--3--", 3 );
...
void card_desbanco(char arquivo_nome[]){
	...
	escreveTextoss(35, 24,"Digite o numero da camisa do jogador que ira escalar para o time principal: ", 3 );
    ...
    if(!achou) escreveTextoss(39, 26,"Nenhum jogador do banco tem o numero digitado", 3 );
