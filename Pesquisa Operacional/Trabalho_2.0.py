...
#---------------------------------------------------------
    ...
    def exibir_sobre_nos(self):
        ...
        #imagem = imagem.resize((200, 200), Image.ANTIALIAS)  # Redimensiona a imagem conforme necessário
    ...
    def __init__(self, master=None):
        ...
        self.master = master
        self.title("Mukifo Solver") # <---- importante
...
#---------Funcoes-----------------------------------------------------------
    ...
    def Central(self):
        ...
        def Mostra_Forma_Padrao(self):
            ...
            forma_padrao_frame = tk.Label(self.master_frame)
            forma_padrao_frame.pack(side="left", padx=100, anchor="nw")
        ...
        def Mostra_Resultado(self, erro, cr):
            ...
            resultado_frame = tk.Label(self.master_frame)
            resultado_frame.pack(side="right", padx=100, anchor="ne")
        ...
        def adicionar_variavel_artificial_igual(coeficientes, objetivo, Base, id_Base):
            ...
            objetivo = np.append(objetivo, 200.0)  

            nova_base = base(id= len(objetivo) , cost = 200)
        ...
        def adicionar_variavel_folga(coeficientes, objetivo, Base, id_Base):
            ...
            nova_base = base(id= len(objetivo) , cost = 0)
        ...
        print("max/min", valor_selecionado)                                                         #mostra  opção escolhda no console
        ...
        print("objetivo", self.objetivo)                                                            #mostra objetivo com operações feitas
        print("qtd de restricoes", self.contador)                                                   #mostra qtd de restrições
        ...
        for entry_group in self.restricoes_entries:
            ...
            print("coeficientes da restricao" , coeficientes)
        ...
        self.limpar_janela()
        self.simplex_photo = tk.PhotoImage(file="simplex.png")      
        self.image_label = tk.Label(self, image=self.simplex_photo)
        self.image_label.pack(pady=10)
        ...
        while(erro == 0):
            erro = 0                                                                                #suposição de que não há erros
            cr = [0] * qtd_restricao                                                                #n° de restrições declaracas acima

            for i in range (qtd_restricao - 1):                                   # mudança em relação ao código original por conta de overflow
            ...
            if valor < 0:
                ...
                if indice >= 0:
                    ...
                    print("Matrix Ax e b interacao qq")
                    for base_atual, linha_Ax, valor_b in zip(self.Base, self.Ax, self.b):
                        print(base_atual.cost, " x" , base_atual.id, " | ",   linha_Ax, " ", valor_b)
